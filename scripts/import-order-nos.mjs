...
const filePath =
  ...
  (fs.existsSync(defaultFullCsv) ? defaultFullCsv : '')

if (!filePath) {
  await seedDemoIfEmpty()
  console.log('用法：npm run import-order-nos -- "物料清单.xlsx"')
  console.log('      npm run import-order-nos -- --replace "物料清单.xlsx"  （整库替换，慎用）')
  process.exit(0)
}
...
if (replaceMode) {
  ...
  const result = importOrderNos(rows)
  console.log(
    `已增量导入。读到 ${rows.length} 行，新增 ${result.inserted} 条（已有订货号跳过）。库内共 ${result.total} 条。`
  )
}
